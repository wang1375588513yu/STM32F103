...
__IO uint16_t ADC_ConvertedValue[50][1];//用来存放ADC转换结果，也是DMA的目标地址。均值滤波，连续采50次求平均值
...
static void ADC1_Mode_Config(void)
{
	...
	DMA_InitStructure.DMA_MemoryBaseAddr = (uint32_t)&ADC_ConvertedValue;	//内存地址，DMA的目标地址二维buf，dma搬运到这
	DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralSRC;
	DMA_InitStructure.DMA_BufferSize = 50;	//每通道采集50次,2个通道,50*2，一次搬运的大小
	DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;	//外设地址固定，外设的地址不要增量
	DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;  			//内存地址递增 ，内存的即DMA的目的buf要增量
	DMA_InitStructure.DMA_PeripheralDataSize = DMA_PeripheralDataSize_HalfWord;	//数据宽度为半字
	DMA_InitStructure.DMA_MemoryDataSize = DMA_MemoryDataSize_HalfWord;	//数据宽度为16位，ADM数据传输数目2^16=65536
	...
	ADC_InitStructure.ADC_ScanConvMode = DISABLE; 	 	//扫描模式，扫描模式用于多通道采集 ENABLE
	...
	/* Enable ADC1 DMA */
	ADC_DMACmd(ADC1, ENABLE);//是adc的数据就给dma
...
//	Value = Value / (0x0FFF + 1);
	Value = (float)temp * (3.3 / 4096);//VDDA引角
