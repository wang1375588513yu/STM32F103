...
uint8_t iokey_table[4][3] = {	//IO按键的几种可能的状态
	{IOKEY_SHORT},	//按一下
	{IOKEY_LONG},	//长按	
	{IOKEY_HOLD},	//按住
	{IOKEY_LONG_UP}	//长按抬起
...
uint8_t adkey_table[4][5] = {	//AD按键的几种可能状态
	{ADKEY_SHORT},
	{ADKEY_LONG},
	{ADKEY_HOLD},
	{ADKEY_LONG_UP}
...
void Key_Scan(void)
{
	...
	if(NO_KEY == keyTemp)
	{
		if (key_counter >= KEY_LONG_CNT)	//如果计数大于等于200
		{
			key_style = 3;
		}
		else if (key_counter >= KEY_BASE_CNT)	//如果计数大于等于35
		{
			key_style = 0;
		}
